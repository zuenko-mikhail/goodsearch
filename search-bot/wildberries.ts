...
function getImgPath(id: number) {
    ...
    return `https://basket-${a}.wbbasket.ru/vol${b}/part${~~(id / 1e3)}/${id}/images/516x688`;
}

export default async function scanWildberries() {
    const session = new Session('recom.wb.ru');
    for (let page = 1; ; page++) {
        const { data } = JSON.parse(await session.get(`/personal/ru/common/v5/search?curr=rub&dest=-366541&page=${page}&query=0&resultset=catalog`));
